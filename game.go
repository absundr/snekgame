...
func drawParts(s tcell.Screen, parts []SnakePart, foodPos SnakePart, snakeStyle tcell.Style, foodStyle tcell.Style) {
...
func drawText(s tcell.Screen, x1, y1, x2, y2 int, text string) {
	row := y1
	col := x1
	style := tcell.StyleDefault.Background(tcell.ColorAliceBlue).Foreground(tcell.ColorPeachPuff)
	for _, r := range text {
